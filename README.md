...
1. RISC-V GNU toolchain needs to be downloaded from: https://github.com/riscv-collab/riscv-gnu-toolchain. It follows standard build process as well. Should any compilation error occur, it should be reported and fixed by upstream. The following commit had was tested to build fine with `gcc-15` and `g++-15`: `bca3efed29ea`. If make returns `429` errors, try using the github mirror for all the submodules. Following configuration should be used: `./configure --prefix=/your/install/dir --with-arch=rv32imcb --with-abi=ilp32 --enable-multilib` and then `make` the toolchain.
